...
export class CompanyService {
  ...
  static async getCompanyById(id: string): Promise<ApiResponse<Company>> {
    ...
    if (data) {
      ...
      if (!company) {
        return { success: false, message: `Company with id '${id}' not found`, status: ApiStatus.NOT_FOUND };
      }
  ...
  static async updateCompany(id: string, company: Partial<Company>, userId: string): Promise<ApiResponse<Company>> {
    if (company.code && (await this.isCompanyCodeExists(company.code, id))) {
      return { success: false, message: `Company with code '${company.code}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
    if (company.name && (await this.isCompanyNameExists(company.name, id))) {
    ...
    const updateCompany = { ...dbCompany, ...company, address, contact, users: auditUsers };
    const parsedResult = CompanySchema.safeParse(updateCompany);
    if (!parsedResult.success) {
      return { success: false, message: parsedResult.error.message, status: ApiStatus.BAD_REQUEST };
    }
    const updatedCompany = await this.companyRepository.save(parsedResult.data as Company);
