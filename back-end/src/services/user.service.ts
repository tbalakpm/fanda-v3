...
export class UserService {
  ...
  static async getUserById(id: string): Promise<ApiResponse<UserDto>> {
    ...
    if (data) {
      ...
      if (!user) {
        return { success: false, message: `User with id '${id}' not found`, status: ApiStatus.NOT_FOUND };
      }
  ...
  static async getLoginByUsername(username: string): Promise<ApiResponse<User>> {
    const user = await this.userRepository.findOne({
      where: { username },
      select: ["id", "username", "password", "email", "phone", "firstName", "lastName", "role", "isActive"]
    });
    if (!user) {
  ...
  static async updateUser(id: string, user: Partial<UserDto>): Promise<ApiResponse<UserDto>> {
    if (user.username && (await this.isUsernameExists(user.username, id))) {
      return { success: false, message: `Username with name '${user.username}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
