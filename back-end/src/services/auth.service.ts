...
export class AuthService {
  static async register(register: User): Promise<AuthResponse> {
    ...
    const token = encrypt.generateToken({ userId: (result.data as User).userId });
    return { ...result, token };
  }
  ...
  static async login(login: LoginDto): Promise<AuthResponse> {
    ...
    if (!result.success || !result.data) {
      return { success: false, message: "Login failed, invalid credentials", status: ApiStatus.UNAUTHORIZED };
    }
    ...
    return { success: true, message: "Login successful", token, data: userWithoutPassword as User, status: ApiStatus.OK };
  }
}
