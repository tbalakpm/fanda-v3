...
import { FilterRule, type GetAllQuery, type Pagination } from '../interfaces/get-all-query';
...
export const getSorting = (sort?: string) => {
  ...
  if (!sortValue.match(sortPattern)) {
...
export const getFiltering = (filter?: string) => {
  ...
  if ((filters.length === 2 && !filters[1].match(/^(isnull|isnotnull)$/)) || !filters[1].match(/^(eq|neq|gt|gte|lt|lte|like|nlike|in|nin)$/)) {
    filterStr = `${filters[0]}:eq:${filters[1]}`;
  }
  ...
  if (!filterStr.match(filterPattern) && !filterStr.match(nullFilterPattern)) {
  ...
  const [property, rule, value] = filterStr.split(':');
