...
export class ProductService {
  ...
  static async getAllProducts(companyId: string): Promise<ApiResponse<Product[]>> {
    ...
    if (data) {
      return { success: true, message: "Serving products from cache", data, status: ApiStatus.OK };
    }
    ...
    return { success: true, message: "Serving products from database", data: products, status: ApiStatus.OK };
  }
  ...
  static async getProductById(companyId: string, productId: string): Promise<ApiResponse<Product>> {
    ...
    if (data) {
      return { success: true, message: "Serving a product from cache", data, status: ApiStatus.OK };
    }
    ...
    if (!product) {
      return { success: false, message: `Product with id '${productId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    await cache.set("products:" + productId, product);
    return { success: true, message: "Serving product from database", data: product, status: ApiStatus.OK };
  }
  ...
  static async createProduct(companyId: string, product: Product, userId: string): Promise<ApiResponse<Product>> {
    ...
    if (product.code && (await this.isProductCodeExists(companyId, product.code))) {
      return { success: false, message: `Product with code '${product.code}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
    if (product.name && (await this.isProductNameExists(companyId, product.name))) {
      return { success: false, message: `Product with name '${product.name}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
    ...
    return { success: true, message: "Product created successfully", data: newProduct, status: ApiStatus.CREATED };
  }
  ...
  static async updateProduct(companyId: string, productId: string, product: Partial<Product>, userId: string): Promise<ApiResponse<Product>> {
    const dbProduct = await this.productRepository.findOneBy({ companyId, productId });
    if (!dbProduct) {
      return { success: false, message: `Product with id '${productId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    ...
    return { success: true, message: "Product updated successfully", data: updatedProduct, status: ApiStatus.OK };
  }

  static async deleteProduct(companyId: string, productId: string): Promise<ApiResponse<Product>> {
    const product = await this.productRepository.findOneBy({ companyId, productId });
    if (!product) {
      return { success: false, message: `Product with id '${productId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    await this.productRepository.remove(product);
    this.invalidateCache(product.companyId, productId);
    return { success: true, message: "Product deleted successfully", data: product, status: ApiStatus.OK };
  }

  static async isProductCodeExists(companyId: string, productCode: string, productId?: string): Promise<boolean> {
    let exists = true;
    if (productId) {
      exists = await this.productRepository.exists({ where: { companyId: companyId, code: productCode, productId: Not(productId) } });
      return exists;
    } else {
      exists = await this.productRepository.existsBy({ companyId: companyId, code: productCode });
      return exists;
    }
  }
  ...
  static async isProductNameExists(companyId: string, productName: string, productId?: string): Promise<boolean> {
    let exists = true;
    if (productId) {
      exists = await this.productRepository.exists({ where: { companyId: companyId, name: productName, productId: Not(productId) } });
      return exists;
    } else {
      exists = await this.productRepository.existsBy({ companyId: companyId, name: productName });
      return exists;
    }
  }
