...
export class ProductCategoryService {
  ...
  static async getAllCategories(companyId: string): Promise<ApiResponse<ProductCategory[]>> {
    ...
    if (data) {
      return { success: true, message: "Serving categories from cache", data, status: ApiStatus.OK };
    }
    ...
    return { success: true, message: "Serving categories from database", data: categories, status: ApiStatus.OK };
  }
  ...
  static async getCategoryById(companyId: string, categoryId: string): Promise<ApiResponse<ProductCategory>> {
    ...
    if (data) {
      return { success: true, message: "Serving a category from cache", data, status: ApiStatus.OK };
    }
    ...
    if (!category) {
      return { success: false, message: `Category with id '${categoryId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    await cache.set("product_categories:" + categoryId, category);
    return { success: true, message: "Serving category from database", data: category, status: ApiStatus.OK };
  }
  ...
  static async createCategory(companyId: string, category: ProductCategory, userId: string): Promise<ApiResponse<ProductCategory>> {
    ...
    if (category.code && (await this.isCategoryCodeExists(companyId, category.code))) {
      return { success: false, message: `Category with code '${category.code}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
    if (category.name && (await this.isCategoryNameExists(companyId, category.name))) {
      return { success: false, message: `Category with name '${category.name}' already exists`, status: ApiStatus.BAD_REQUEST };
    }
    ...
    return { success: true, message: "Category created successfully", data: newCategory, status: ApiStatus.CREATED };
  }
  ...
  static async updateCategory(
    ...
    const dbCategory = await this.categoryRepository.findOneBy({ companyId, categoryId });
    if (!dbCategory) {
      return { success: false, message: `Category with id '${categoryId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    ...
    return { success: true, message: "Category updated successfully", data: updatedCategory, status: ApiStatus.OK };
  }

  static async deleteCategory(companyId: string, categoryId: string): Promise<ApiResponse<ProductCategory>> {
    const category = await this.categoryRepository.findOneBy({ companyId, categoryId });
    if (!category) {
      return { success: false, message: `Category with id '${categoryId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    await this.categoryRepository.remove(category);
    this.invalidateCache(category.companyId, categoryId);
    return { success: true, message: "Category deleted successfully", data: category, status: ApiStatus.OK };
  }

  static async isCategoryCodeExists(companyId: string, categoryCode: string, categoryId?: string): Promise<boolean> {
    let exists = true;
    if (categoryId) {
      exists = await this.categoryRepository.exists({ where: { companyId: companyId, code: categoryCode, categoryId: Not(categoryId) } });
      return exists;
    } else {
      exists = await this.categoryRepository.existsBy({ companyId: companyId, code: categoryCode });
      return exists;
    }
  }
  ...
  static async isCategoryNameExists(companyId: string, categoryName: string, categoryId?: string): Promise<boolean> {
    let exists = true;
    if (categoryId) {
      exists = await this.categoryRepository.exists({ where: { companyId: companyId, name: categoryName, categoryId: Not(categoryId) } });
      return exists;
    } else {
      exists = await this.categoryRepository.existsBy({ companyId: companyId, name: categoryName });
      return exists;
    }
  }
