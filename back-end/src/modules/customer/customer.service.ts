...
export class CustomerService {
  ...
  static async getAllCustomers(companyId: string): Promise<ApiResponse<Customer[]>> {
    ...
    return { success: true, message: "Serving customers from database", data: customers, status: ApiStatus.OK };
  }
  ...
  static async getCustomerById(companyId: string, customerId: string): Promise<ApiResponse<Customer>> {
    ...
    return { success: true, message: "Serving customer from database", data: customer, status: ApiStatus.OK };
  }
  ...
  static async createCustomer(companyId: string, customer: Customer, userId: string): Promise<ApiResponse<Customer>> {
    ...
    return { success: true, message: "Customer created successfully", data: newCustomer, status: ApiStatus.CREATED };
  }
  ...
  static async updateCustomer(companyId: string, customerId: string, customer: Partial<Customer>, userId: string): Promise<ApiResponse<Customer>> {
    const dbCustomer = await this.customerRepository.findOneBy({ companyId, customerId });
    if (!dbCustomer) {
      return { success: false, message: `Customer with id '${customerId}' not found`, status: ApiStatus.NOT_FOUND };
    }
    ...
    return { success: true, message: "Customer updated successfully", data: updatedCustomer, status: ApiStatus.OK };
  }

  static async deleteCustomer(companyId: string, customerId: string): Promise<ApiResponse<Customer>> {
    const customer = await this.customerRepository.findOneBy({ companyId, customerId });
    if (!customer) {
      return { success: false, message: `Customer with id '${customerId}' not found`, status: ApiStatus.NOT_FOUND };
    }
  ...
  static async isCustomerCodeExists(companyId: string, customerCode: string, customerId?: string): Promise<boolean> {
    let exists = true;
    if (customerId) {
      exists = await this.customerRepository.exists({ where: { companyId: companyId, code: customerCode, customerId: Not(customerId) } });
      return exists;
    } else {
  ...
  static async isCustomerNameExists(companyId: string, customerName: string, customerId?: string): Promise<boolean> {
    let exists = true;
    if (customerId) {
      exists = await this.customerRepository.exists({ where: { companyId: companyId, name: customerName, customerId: Not(customerId) } });
      return exists;
    } else {
