...
class SalesService {
  ...
  async getAllSales(companyId: string, yearId: string, query: GetAllQuery): Promise<ApiResponse<Sales[]>> {
    // const data = await cache.get<Sales[]>(`saless_${companyId}_${yearId}`);
    // if (data) {
    //   return { success: true, message: 'Serving saless from cache', data, status: ApiStatus.OK };
    // }
    ...
    await cache.set(`saless_${companyId}_${yearId}`, invoices);
    return { success: true, message: 'Serving saless from database', data: invoices, status: ApiStatus.OK };
  }

  async getSalesById(companyId: string, yearId: string, invoiceId: string): Promise<ApiResponse<Sales>> {
    const data = await cache.get<Sales>(`saless_${companyId}_${yearId}:${invoiceId}`);
    if (data) {
    ...
    await cache.set(`saless_${companyId}_${yearId}:${invoiceId}`, invoice);
  ...
  async invalidateCache(companyId: string, yearId: string, invoiceId?: string): Promise<void> {
    await cache.del(`saless_${companyId}_${yearId}`);
    if (invoiceId) {
      await cache.del(`saless_${companyId}_${yearId}:${invoiceId}`);
    }
  }
}
